...
export default function PredictionsPage() {
  ...
  useEffect(() => {
    ...
    fetchPredictionFromAI(ipk, targetAttendance, targetAssignment, quizScore, atsScore);
  ...
  const handleSliderChange = (type: string, val: number) => {
    ...
    fetchPredictionFromAI(newIpk, newAttendance, newAssignment, newQuiz, newAts);
  };
      ...
      >
        <div>
          ...
          <nav className="space-y-1">
            <a
              href="/dashboard"
              className="flex items-center gap-3 px-4 py-3 rounded-xl text-[#516070] hover:bg-[#eff4ff] hover:text-[#0b1c30] transition font-medium text-sm"
            >
              <span>📊</span> Dashboard
            </a>
            <a
            ...
            <a
              href="/predictions"
              className="flex items-center gap-3 px-4 py-3 rounded-xl bg-[#e5eeff] text-[#004ac6] font-semibold text-sm"
            >
        ...
        <div className="p-4 md:p-8 space-y-4 md:space-y-8 max-w-7xl mx-auto w-full">
          <div>
            <h3 className="text-xl md:text-2xl font-extrabold text-[#0b1c30]">
              Interactive 5-Variables Predictor
            </h3>
            <p className="text-xs md:text-sm text-[#434655] mt-1">
              Gunakan simulator ini untuk melihat bagaimana IPK, Absensi, Tugas, Kuis, dan ATS
              saling mempengaruhi probabilitas keberhasilan studi.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
            ...
            <div className="bg-white border border-[#c3c6d7]/40 rounded-2xl p-5 md:p-8 shadow-xs lg:col-span-2 space-y-5 md:space-y-6">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-[#c3c6d7]/30 pb-4 gap-3">
                <div>
                  ...
                  <p className="text-[11px] md:text-xs text-[#434655]">
                    Sesuaikan nilai untuk melihat kalkulasi AI secara real-time.
                  </p>
                </div>
              ...
              <div className="space-y-2 bg-blue-50/50 p-4 rounded-xl border border-blue-100">
                ...
                <input
                  ...
                  step="0.01"
                  value={ipk}
                  onChange={(e) => handleSliderChange('ipk', parseFloat(e.target.value))}
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-xs md:text-sm font-semibold">
                  <span className="text-[#0b1c30]">Persentase Kehadiran</span>
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-xs md:text-sm font-semibold">
                  <span className="text-[#0b1c30]">Nilai Rata-rata Tugas</span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-6 pt-2">
                ...
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    max="100"
                    value={quizScore}
                    onChange={(e) => handleSliderChange('quiz', Number(e.target.value))}
                    className="w-full accent-[#004ac6] cursor-pointer"
                  />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    max="100"
                    value={atsScore}
                    onChange={(e) => handleSliderChange('ats', Number(e.target.value))}
                    className="w-full accent-[#004ac6] cursor-pointer"
                  />
                </div>
              </div>
            ...
            <div className="bg-white border border-[#c3c6d7]/40 rounded-2xl p-5 md:p-8 shadow-xs lg:col-span-1 flex flex-col justify-between">
              <div>
                <h4 className="font-bold text-base md:text-lg text-[#0b1c30] mb-1 md:mb-2">
                  Predicted Outcome
                </h4>
                <p className="text-[11px] md:text-xs text-[#434655] mb-6">
                  Skor risiko ditenagai oleh Random Forest Engine.
                </p>

                <div className="flex flex-col items-center justify-center my-6">
                  <div
                    className={`w-32 h-32 md:w-44 md:h-44 rounded-full border-[6px] md:border-8 ${circleColor} bg-[#f8f9ff] flex flex-col items-center justify-center shadow-inner transition-colors duration-500`}
                  >
                    <span className="text-3xl md:text-5xl font-extrabold text-[#0b1c30]">
                      {predictedScore}%
                    </span>
                    <span
                      className={`text-[9px] md:text-xs ${statusTextColor} font-bold uppercase tracking-wider mt-1 transition-colors duration-500`}
                    >
                      {riskStatus}
                    </span>
                  </div>
                ...
                <div className={`p-4 mt-6 rounded-xl border ${badgeBg} transition-colors duration-300`}>
                  <p className={`text-[11px] md:text-xs font-bold uppercase tracking-wider mb-2 ${statusTextColor}`}>
                    💡 AI Analysis
                  </p>
